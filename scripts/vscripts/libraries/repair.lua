...
function BuildingHelper:OnPreRepair(builder, target)
    ...
    local bValidRepair = (
        (
            target:GetClassname() == "npc_dota_building" or target:GetClassname() == "npc_dota_fort"
        )
            and IsCustomBuilding(target)
            and target:GetHealthPercent() < 100
        )
...
function BuildingHelper:OnRepairTick(building, hpGain, costFactor)
    print("OnRepairTick",building,hpGain,costFactor)
