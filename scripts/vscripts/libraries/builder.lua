...
function Build( event )
    ...
    -- Added by SirWayNe - No Gold Cost, since every Gold Cost Ability is Custom Ability Panel -> No Gold paid yet
    -- hero:ModifyGold(gold_cost, false, 0)

    -- Added by SirWayNe: Add ManaCost back to the Hero
    -- Removed since this costs energy rather then mana now
    --[[
    local mana_cost = ability:GetManaCost(1)
    hero:ModifyEnergy( mana_cost )
    -- print("Build: Check for Correct Mana Cost usage")
    --]]
    -- End SirWayNe

    -- Makes a building dummy and starts panorama ghosting
    BuildingHelper:AddBuilding(event)
    BuildingHelper:print( "Builder: AddBuilding DONE. Setting Events..." )
    ...
    event:OnPreConstruction(function(vPos)
            ...
            -- If not enough resources to queue, stop
            if hero:GetEnergy() < energy_cost then
                BuildingHelper:print("Failed placement of " .. building_name .." - Not enough mana!")
                SendErrorMessage(playerID, "#error_not_enough_energy")
                return false
            end
            if hero:GetEssence() < essence_cost then
                BuildingHelper:print("Failed placement of " .. building_name .. " - Not enough Essence!")
                SendErrorMessage(playerID, "#error_not_enough_essence")
                return false
            end
            BuildingHelper:print("Building is fine here: " .. building_name )
    ...
    event:OnBuildingPosChosen(function(vPos)
        ...
        hero:ModifyEnergy(-energy_cost)
        hero:ModifyEssence(-essence_cost)
    ...
    event:OnConstructionFailed(function()
        ...
        -- SendErrorMessage(playerID, "#error_not_buildable_here")
        SendErrorMessage(playerID, "#error_invalid_build_position")
    end)
    ...
    event:OnConstructionStarted(function(unit)
        ...
        CCastleFightReforged:OnBuildingStarted( {
            unit = unit,
            duration = ability:GetKeyValue("BuildTime"),
            playerID = playerID
        } )
    end)
    ...
    event:OnConstructionCompleted(function(unit)
        ...
        CCastleFightReforged:OnBuildingFinished( { unit = unit } )
    end)
...
function CancelBuilding( keys )
    ...
    if building.gold_cost then
        ...
        -- hero:SetMana( hero:GetMana() + mana_cost )
    end
