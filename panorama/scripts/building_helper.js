...
function RegisterGNV(msg){
    ...
    for (var i=0; i<msg.gnv.length; i++){
        // loop through every entry of GNV
        // var code = msg.gnv.charCodeAt(i);
        var code = msg.gnv.charCodeAt(i)-32;
        for (var j=4; j>=0; j-=2){
            var g = (code & (3 << j)) >> j;
            // $.Msg(i, " code: ",code," g: ",g, " index ",j)
            x++
            if (g != 0)
			{
              arr.push(g);
    ...
    for (var i = 0; i < squareY; i++) {
        ...
        for (var j = 0; j < squareX; j++) {
          ...
          x++
		  // $.Msg('GridNav: Grid: ',i,' ',j,' Arr: ',arr[x])
        }
    }
    // $.Msg('Total Grid Size: ',x)
    ...
    // $.Msg("Free: ",tab["1"]," Blocked: ",tab["2"],' Total Squares: ',x)
}
...
function IsBlocked(position) {
	...
	// $.Msg('IsBlocked: Position:',x,' ',y,' Flag: ',flag)
    ...
    // $.Msg('GRID:',Root.GridNav[x][y],' ENTGRID:',entGridValue,' FLAG:',flag,' REQUIRES:', requires,' Position:', position[0],' ',position[1],' ',position[2])
	
	
    // If the bits don't match, its invalid
    if ((flag & requires) != requires)
	{
		// $.Msg('GRID: failed with required Flags')
        return true // SirWayNe 25.02.2019: disabled. Doesnt work properly in Panorama
	}

    // If there's a tree standing, its invalid
    if (update_trees && treeGrid[x] && (treeGrid[x][y] & GRID_TYPES["BLOCKED"]))
	{
		// $.Msg('GRID: failed with Tree in Position')
        return true
	}
